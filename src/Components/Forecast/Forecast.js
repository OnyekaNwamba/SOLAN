...
const API_KEY = "ae044746765db55ca524cc294e0d1a3a";
...
//const API_KEY = "e9368557fb1b1d3a868779c8008667a9";
...
function getNextDayForecast(json){
    ...
    // Get avg temp, wind speed, humidity, weather cond for next five days (3 hour interval)
    // Get min temp and max temp for next five days

    for(let i = 0;i<json.list.length;i++){
        let day = getTimestampDay(json.list[i].dt);

        // Skip the first day, get the forecast for the next days
        if(day == firstDay)
            continue;
    ...
    for(let j = 0; j<keys.length;j++){
        data.push({
            day: keys[j],
            // Get average temp, wind, humidity, weather code
